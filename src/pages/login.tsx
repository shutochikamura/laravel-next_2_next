...
const Login = () => {
    ...
    useEffect(() => {
        if (router.query.reset !== undefined && router.query.reset?.length > 0 && errors.length === 0) {
            const decodedData: string = window.atob(String(router.query.reset))
            setStatus(decodedData)
        } else {
            setStatus(null)
        }
    })
